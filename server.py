...
def pack_handshake(message, crypto_dict, verbose = False):
	...
	debug(verbose, "aes_session_key", crypto_dict["aes_session_key"])

	# Compile message
	client_message = message + "," + crypto_dict["aes_session_key"]
	...
	# Sign message
	try:
		message_signature = str(rsa.sign(crypto_dict["rsa_server_private_key"], message_hash))
	except Exception as inst:
	...
	try:
		...
		for i in range(0, num_message_parts):
			rsa_encrypted_messages.append(str(rsa.encrypt(crypto_dict["rsa_user_public_key"], signed_message[ (i * 256) : (i + 1) * 256 ])))
	except Exception as inst:
...
def pack_message(message, crypto_dict, verbose = False):
	...
	# Nonce
	nonced_message = server_message + "," + crypto_dict["client_nonce"] + "," + crypto_dict["server_nonce"] + ","
...
def unpack_handshake(encoded_message, crypto_dict, verbose = False):
	...
	# Verify Signature
	if str(rsa.unsign(crypto_dict["rsa_user_public_key"], message_signature)) != message_hash:
...
def unpack_message(encoded_message, crypto_dict, verbose = False):
	...
	mcSplit = message_content.split(",")
	message, auth_username, auth_password, client_nonce, server_nonce = mcSplit[0], mcSplit[1], mcSplit[2], mcSplit[3], mcSplit[4]
	...
	# Verify Nonce
	if server_nonce != crypto_dict["server_nonce"]:
...
handshake = pack_handshake(message, crypto_dictionary, GLOBAL_VERBOSE)
if handshake[0]:
	try:
...
message = pack_message(message, crypto_dictionary, GLOBAL_VERBOSE)
if message[0]:
	try:
